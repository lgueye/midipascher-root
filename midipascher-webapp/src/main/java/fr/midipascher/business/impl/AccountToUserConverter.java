...
public class AccountToUserConverter implements Converter<Account, User> {
	...
	public User convert(Account source) {
		...
		Collection<? extends GrantedAuthority> authorities = fromAuthorities(source.getAuthorities());
	 ...
	 * @return
	 */
	private Collection<GrantedAuthority> fromAuthorities(Set<Authority> authorities) {

		Collection<GrantedAuthority> collection = new ArrayList<GrantedAuthority>();

		for ( Authority authority : authorities )
			collection.add(this.authorityToGrantedAuthorityConverter.convert(authority));
