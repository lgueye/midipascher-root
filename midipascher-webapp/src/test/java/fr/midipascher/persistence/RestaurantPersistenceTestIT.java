...
public class RestaurantPersistenceTestIT extends BasePersistenceTestIT {
	...
	private void assertRestaurantHasSpecialties(final Restaurant restaurant, final Set<Long> ids) {
		if (restaurant == null && ids == null) return;
	...
	public void shouldCreateRestaurant() {
		...
		Assert.assertEquals(restaurant, this.baseDao.get(Restaurant.class, restaurant.getId()));
	}
	...
	public void shouldDeleteRestaurant() {
		...
		this.baseDao.persist(restaurant);
		this.baseDao.flush();
		this.baseDao.delete(Restaurant.class, restaurant.getId());
		this.baseDao.flush();

		// Then
		final Restaurant persistedRestaurant = this.baseDao.get(Restaurant.class, restaurant.getId());
	...
	public void shouldFindRestaurantByCityAndPostalCode() {
		...
		// When
		final List<Restaurant> results = this.restaurantDao.findByExample(restaurant);
	...
	public void shouldFindRestaurantByHalalCharacteristic() {
		...
		final Restaurant persistedRestaurant = this.baseDao.get(Restaurant.class, persistedRestaurantId);
		persistedRestaurant.setHalal(true);
		this.baseDao.persist(persistedRestaurant);
		this.baseDao.flush();
		final Restaurant restaurant = new Restaurant();
		restaurant.setHalal(true);

		// When
		final List<Restaurant> results = this.baseDao.findByExample(restaurant);
	...
	public void shouldUpdateRestaurant() {
		...
		this.baseDao.persist(restaurant);
		this.baseDao.flush();
		this.baseDao.evict(restaurant);
		final String name = RandomStringUtils.random(50);
		restaurant.setName(name);

		// When
		this.baseDao.merge(restaurant);
		this.baseDao.flush();
		final Restaurant persistedRestaurant = this.baseDao.get(Restaurant.class, restaurant.getId());
	...
	public void shouldUpdateRestaurantFoodSpecialties() {
		...
		final Restaurant restaurant = this.baseDao.get(Restaurant.class, 1L);
		...
		final FoodSpecialty sp4 = this.baseDao.get(FoodSpecialty.class, 4L);
		Assert.assertNotNull(sp4);
		final FoodSpecialty sp5 = this.baseDao.get(FoodSpecialty.class, 5L);
		Assert.assertNotNull(sp5);
		final FoodSpecialty sp2 = this.baseDao.get(FoodSpecialty.class, 2L);
		Assert.assertNotNull(sp5);
	...
	private Restaurant validRestaurant() {
		...
		final FoodSpecialty foodSpecialty = this.baseDao.get(FoodSpecialty.class, 1L);
